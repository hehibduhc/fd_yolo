...
def read_image_size(root: ET.Element) -> tuple[int | None, int | None]:
    size = root.find("size")
    if size is None:
        return None, None
    try:
...
def build_argparser():
    p = argparse.ArgumentParser(
        description="Convert VOC XML <robndbox> to normalized quads: 'class_index x1 y1 x2 y2 x3 y3 x4 y4'"
    )
