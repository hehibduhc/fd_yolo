...
def load_name_to_index(mapping_path: Optional[Path]):
...
def convert_dir_xml_to_obb_txt(src_dir: Path, out_dir: Path, classes_path=None,
                               angle_in_degrees_in_xml=False, reverse_angle_sign=False,
                               normalize=False, default_wh=None, emit_degrees=False,
                               mirror_subdirs=True, default_unknown=-1, strict_unknown=False,
                               single_file=None):
    ...
    for xml_path in sorted(src_dir.rglob("*.xml")):
        dst = (out_dir / (xml_path.relative_to(src_dir).with_suffix(".txt"))) if (not single_file and mirror_subdirs) else (out_dir / (xml_path.stem + ".txt") if not single_file else out_dir / single_file)
        lines = parse_one_xml(xml_path, name_to_index, default_unknown, strict_unknown,
                              angle_in_degrees_in_xml, reverse_angle_sign, normalize, default_wh, emit_degrees)
        if single_file:
            merged.extend(lines)
        else:
...
def main():
    ...
    res = convert_dir_xml_to_obb_txt(
        ...
        angle_in_degrees_in_xml=args.angle_deg, reverse_angle_sign=args.reverse_angle,
        normalize=args.normalize, default_wh=default_wh, emit_degrees=args.emit_deg,
