...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Directory containing label files (.txt YOLO or .xml robndbox)",
    )
...
def _load_xml_labels(label_path: Path) -> list[str]:
    ...
    for obj in root.findall("object"):
        name = obj.findtext("name", default="0").strip()
        ...
        w = rbox.findtext("w")
        h = rbox.findtext("h")
        angle = rbox.findtext("angle")
        if None in {cx, cy, w, h}:
            continue

        parts = [name, cx, cy, w, h]
...
def load_labels(label_path: Path) -> list[str]:
    if label_path.suffix.lower() == ".xml":
        return _load_xml_labels(label_path)
    return _load_txt_labels(label_path)
...
def main() -> None:
    ...
    label_files = sorted([p for p in Path(args.labels).iterdir() if p.suffix.lower() in {".txt", ".xml"}])
    if not label_files:
    ...
    for label_path in label_files:
        ...
        out_name = f"{label_path.stem}.txt"
